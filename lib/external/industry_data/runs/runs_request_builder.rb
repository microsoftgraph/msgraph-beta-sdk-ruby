...
require_relative '../../../models/industry_data/industry_data_run_collection_response'
        ...
        module IndustryData
            module Runs
                ...
                class RunsRequestBuilder < MicrosoftKiotaAbstractions::BaseRequestBuilder
                    ...
                    def count()
                        return MicrosoftGraphBeta::External::IndustryData::Runs::Count::CountRequestBuilder.new(@path_parameters, @request_adapter)
                    end
                    ## 
                    # Provides operations to call the getStatistics method.
                    def industry_data_get_statistics()
                        return MicrosoftGraphBeta::External::IndustryData::Runs::IndustryDataGetStatistics::IndustryDataGetStatisticsRequestBuilder.new(@path_parameters, @request_adapter)
                    end
                    ## 
                    ## Provides operations to manage the runs property of the microsoft.graph.industryData.industryDataRoot entity.
                    ## @param industry_data_run_id Unique identifier of the item
                    ## @return a industry_data_run_item_request_builder
                    ## 
                    def by_industry_data_run_id(industry_data_run_id)
                        raise StandardError, 'industry_data_run_id cannot be null' if industry_data_run_id.nil?
                    ...
                    ## @return a Fiber of industry_data_run_collection_response
                    ## 
                    def get(request_configuration=nil)
                        ...
                        return @request_adapter.send_async(request_info, lambda {|pn| MicrosoftGraphBeta::Models::IndustryData::IndustryDataRunCollectionResponse.create_from_discriminator_value(pn) }, error_mapping)
                    end
                    ## 
                    ## Set of ephemeral runs which present the point-in-time that diagnostic state of activities performed by the system. Read-only.
                    ## @param request_configuration Configuration for the request such as headers, query parameters, and middleware options.
