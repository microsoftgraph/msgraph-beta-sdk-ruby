...
module MicrosoftGraphBeta
    module Models
        ...
        class ComplianceManagementPartner < MicrosoftGraphBeta::Models::Entity
            ...
            @windows_enrollment_assignments
            ## 
            ...
            def get_field_deserializers()
                return super.merge({
                    "androidEnrollmentAssignments" => lambda {|n| @android_enrollment_assignments = n.get_collection_of_object_values(lambda {|pn| MicrosoftGraphBeta::Models::ComplianceManagementPartnerAssignment.create_from_discriminator_value(pn) }) },
                    ...
                    "windowsEnrollmentAssignments" => lambda {|n| @windows_enrollment_assignments = n.get_collection_of_object_values(lambda {|pn| MicrosoftGraphBeta::Models::ComplianceManagementPartnerAssignment.create_from_discriminator_value(pn) }) },
                    "windowsOnboarded" => lambda {|n| @windows_onboarded = n.get_boolean_value() },
                })
            end
            ## 
            ...
            def serialize(writer)
                ...
                writer.write_collection_of_object_values("windowsEnrollmentAssignments", @windows_enrollment_assignments)
                writer.write_boolean_value("windowsOnboarded", @windows_onboarded)
            end
            ## 
            ## Gets the windowsEnrollmentAssignments property value. User groups which enroll Windows devices through partner.
            ## @return a compliance_management_partner_assignment
            ## 
            def windows_enrollment_assignments
                return @windows_enrollment_assignments
            end
            ## 
            ## Sets the windowsEnrollmentAssignments property value. User groups which enroll Windows devices through partner.
            ## @param value Value to set for the windows_enrollment_assignments property.
            ## @return a void
            ## 
            def windows_enrollment_assignments=(value)
                @windows_enrollment_assignments = value
            end
            ## 
            ## Gets the windowsOnboarded property value. Partner onboarded for Windows devices.
            ## @return a boolean
            ## 
            def windows_onboarded
                return @windows_onboarded
            end
            ## 
            ## Sets the windowsOnboarded property value. Partner onboarded for Windows devices.
            ## @param value Value to set for the windows_onboarded property.
            ## @return a void
            ## 
            def windows_onboarded=(value)
                @windows_onboarded = value
            end
        end
    end
end
